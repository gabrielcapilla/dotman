...
type
  ...
  ExecutionPlan* = object
    count*: int
    capacity*: int
    opCounts*: array[OperationType, int]
...
proc initExecutionPlan*(capacity: int = 1024): ExecutionPlan =
  ExecutionPlan(
    count: 0,
    capacity: capacity,
    opCounts: default(array[OperationType, int]),
...
proc getOperation*(plan: var ExecutionPlan, index: int): Operation =
...
proc clear*(plan: var ExecutionPlan) =
  plan.count = 0
  plan.opCounts = default(array[OperationType, int])

proc getOperationCountByType*(
    plan: var ExecutionPlan, opType: OperationType
): int {.noSideEffect.} =
  plan.opCounts[opType]

proc getSummary*(plan: var ExecutionPlan): string {.noSideEffect.} =
