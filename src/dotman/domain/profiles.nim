...
proc findProfileId*(data: var ProfileData, name: string): ProfileId {.inline.} =
...
proc findProfileIdSafe*(data: var ProfileData, name: string): ProfileResult =
  if name in data.nameIndex:
    let id = data.nameIndex[name]
    let idx = int32(id)
    if data.active[idx]:
...
proc getProfilePath*(data: var ProfileData, id: ProfileId): string =
  let idx = int32(id)
