...
import ../domain/[types, result], ../platform/path
import ../cli/[cli_ops, colors, help, command_parser, completion]
import ../domain/profiles
import ../ui/status_display
import state
import
  ../operations/
    [add, remove, set_link, unset_link, push, pull, executor, profile_store, scan]
...
proc executeRemove*(state: var AppState, fileName: string) =
  ...
  let plan = remove.planRemoveFile(state.profiles, profileId, fileName)
  discard executor.executePlan(plan, verbose = false)
...
proc executeAdd*(state: var AppState, fileName: string) =
  let profileId = state.ensureProfileInitializedOrQuit(MainProfile)
  let plan = add.planAddFile(state.profiles, profileId, fileName)
  discard executor.executePlan(plan, verbose = false)
  colors.actionSuccess(fmt"Linked {fileName}")

proc executeSet*(state: var AppState, fileName: string) =
  let profileId = state.ensureProfileInitializedOrQuit(MainProfile)
  let plan = set_link.planMoveFileToProfile(state.profiles, profileId, fileName)
  discard executor.executePlan(plan, verbose = false)
...
proc executeUnset*(state: var AppState, fileName: string) =
  let profileId = state.ensureProfileInitializedOrQuit(MainProfile)
  let plan = unset_link.planUnsetFile(state.profiles, profileId, fileName)
  discard executor.executePlan(plan, verbose = false)
  colors.actionSuccess(fmt"Restoring {fileName}")

proc showStatus*(
    profiles: var ProfileData, profile: string, profileId: ProfileId, flags: StatusFlags
) =
  var data = scanProfile(
    profiles,
    profileId,
    ScanConfig(maxFiles: 8192, storeHomePaths: flags.filter != FilterAll),
  )
...
proc executePush*(state: var AppState, profileName: string) =
  ...
  let plan = push.planPushProfile(state.profiles, pushProfileId)
  discard executor.executePlan(plan, verbose = false)
  if plan.count == 1:
...
proc executePull*(state: var AppState, profileName: string) =
  ...
  if pullProfileId == ProfileIdInvalid:
    if not dirExists(getDotmanDir()):
      ...
      quit(1)
    raise ProfileError(msg: "Profile not found: " & profileName)
  let plan = pull.planPullProfile(state.profiles, pullProfileId)
  discard executor.executePlan(plan, verbose = false)
  if plan.count == 1:
...
proc executeDeleteProfile*(profileName: string) =
  profile_store.removeProfile(profileName)
  echo "Removed profile: " & profileName

proc dispatch*(parsed: sink ParsedCommand, state: var AppState) =
  case parsed.command
  of CmdInit:
    executeInit(state)
  of CmdMake:
