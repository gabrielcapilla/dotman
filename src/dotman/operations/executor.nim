import std/os
import ../domain/[execution, result], ../platform/[path, path_safety]
import symlinks
...
proc executeCreateSymlink*(op: Operation): string =
  try:
    symlinks.createLink(op.source, op.dest)
...
proc previewPlan*(plan: sink ExecutionPlan) =
  echo ""
...
proc executePlan*(plan: sink ExecutionPlan, verbose: bool = true): ExecutionResult =
