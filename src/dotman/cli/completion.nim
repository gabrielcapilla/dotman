...
"""
  ...
  result &=
    "complete -c dotman -f -n '__fish_seen_subcommand_from completion' -a 'bash zsh fish'\n"
  ...
  result &= "\n# Make command\n"
  result &=
    "complete -c dotman -f -n '__fish_seen_subcommand_from make' -x -a '' -d 'Profile name'\n"

  result &= "\n# Clone command - second argument is destination\n"
  result &=
    "complete -c dotman -f -n '__fish_seen_subcommand_from clone; and __fish_is_nth_token 4' -x -a '' -d 'Destination profile'\n"
...
when isMainModule:
  ...
  var selectedKind = BashCompletion
  var outputFile = ""
  ...
  for optKind, key, val in getopt():
    ...
    of cmdLongOption, cmdShortOption:
      case key
      of "bash":
        selectedKind = BashCompletion
      of "zsh":
        selectedKind = ZshCompletion
      of "fish":
        selectedKind = FishCompletion
      of "output", "o":
        outputFile = val
      of "help", "h":
  ...
  let script = generateCompletion(selectedKind)
