...
proc parseCommand*(argv: openArray[string]): ParsedCommand =
  ...
  while true:
    ...
    of cmdShortOption, cmdLongOption:
      ...
      of "completion":
        ...
        p.next()
        if p.kind in {cmdShortOption, cmdLongOption, cmdArgument}:
          case p.key
          of "bash":
            result.completionKind = BashCompletion
          of "zsh":
...
proc parseCommand*(): ParsedCommand =
  parseCommand(commandLineParams())
